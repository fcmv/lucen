...
- Two performance claims in the README were wrong and are corrected against
  `BENCHMARK.md`. The speedup over Lucen's own sequential execution is 2.5x to
  4.3x, not 3x to 4.3x; `nested heavy` measures 2.54x on 3.12. Lucen does not
  land "within a few percent" of hand-tuned `concurrent.futures` code: across
  the 28 cells the gate parallelizes it is 18 percent slower at the median and
  between 5 and 67 percent slower. The light-reduction probe overhead is 4 to
  14 percent, not 5 to 10.
- The supported-interpreter table reported CPython 3.13t as tested. Only 3.14t
  has a CI job; the table now states what is gated.
- The `lucen explain` samples in the README are captured from real runs. The
  refusal sample showed a mark and a reason string the tool does not emit.
- The guides at the repository root (limitations, benchmarks, stability policy,
  roadmap, changelog, support, contributing) render on the documentation site
  instead of linking back to GitHub.
- `assets/lucen.gif` shows `lucen run`, which replaced the `python demo.py`
  recording made before that command existed, and is regenerated by
